...
export default class GameRepository implements IGameRepository {
    ...
    async getBotById(botId: number): Promise<IBotModel> {
        try {
            const bot: IBotModel = await db.func(storedProcedureNames.GET_BOT_BY_ID, [botId])
            return bot
        }
        catch (error: any) {
            logger.error(error.response.data || `get player skill errors with id: ${botId}`)
            throw new ApplicationError(400, "Method not implemented.");
        }
    }
    async getPlayerSkillById(playerId: number): Promise<IPlayerModel> {
        try {
            ...
            let player: IPlayerModel = { playerId: playerId, userName: "", skills: [] }
            if (result.length > 0) {
